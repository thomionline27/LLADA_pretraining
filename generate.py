...
def main(): 
    ...
    model = MMadaModelLM.from_pretrained("/inspire/hdd/project/embodied-multimodality/public/downloaded_ckpts/MMaDA-8B-Base", trust_remote_code=True, torch_dtype=torch.bfloat16).to(device).eval()
    tokenizer = AutoTokenizer.from_pretrained("/inspire/hdd/project/embodied-multimodality/public/downloaded_ckpts/MMaDA-8B-Base", trust_remote_code=True)
    ...
    out = generate(model, input_ids, steps=16, gen_length=128, block_length=128, temperature=1, cfg_scale=0., remasking='low_confidence')
