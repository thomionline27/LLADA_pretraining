...
class JsonlDataset(IterableDataset):
    """Dataset for reading JSONL files with content field"""
    def __init__(self,
                 data_path,
                 rank: int = 0,
                 world_size: int = 1,
                 ...
                 num_workers=1):
        super().__init__()
        # Support both single file and directory with multiple jsonl files
        ...
        self.files = self.files[self.rank::self.world_size]
        print(f"Rank {self.rank}: Loading {len(self.files)} files")

    def read_jsonl_file(self, file_path):
        """Read JSONL file and yield content field"""
        try:
    ...
    def __iter__(self):
        while True:
            ...
            for file_path in file_list:
                ...
                data_generator = self.read_jsonl_file(file_path)
                buffer = []
                ...
                for data in data_generator:
                    ...
                    if isinstance(text, str):
                        ...
                        if len(text) > self.max_length:
                            # For code data, try to keep complete functions/classes
                            start_index = random.randint(0, max(0, len(text) - self.max_length))
                            selected_text = text[start_index:start_index + self.max_length]
                        else:
                            selected_text = text
                        
                        if len(selected_text.strip()) > 50:  # Filter out very short content
                            buffer.append({"input_ids": selected_text})
