...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            # Ensure attn_mask is on the same device as q/k/v
    ...
    def attention(
        ...
        if attention_bias is not None:
            ...
            attention_bias = self._cast_attn_bias(
                attention_bias[:, :, key_len - query_len : key_len, :key_len].to(device=q.device), dtype
            )
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None and 0.0 in attention_mask:
            ...
            attention_mask = attention_mask.to(device=x.device, dtype=torch.float).view(batch_size, -1)[:, None, None, :]
        ...
        if (
            ...
            if attention_bias is None and self.config.alibi:
                ...
                attention_bias = attention_bias.to(device=x.device, dtype=torch.float)
            ...
            attention_bias = attention_bias[:, :, :mask_len, :mask_len].to(device=x.device, dtype=torch.float)
